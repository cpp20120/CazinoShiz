    ...
    ILogger<DartsHandler> logger) : IUpdateHandler
{
    public async Task HandleAsync(UpdateContext ctx)
    {
        ...
        if (msg.Dice?.Emoji == "🎯")
        {
            await HandleThrowAsync(ctx, msg, userId, displayName, chatId, reply);
            return;
        }
        ...
        string[] parts, ReplyParameters reply)
    {
        ...
        {
            DartsBetError.None => string.Format(Loc("bet.accepted"), r.Amount),
            DartsBetError.InvalidAmount => Loc("bet.invalid"),
            DartsBetError.NotEnoughCoins => string.Format(Loc("bet.not_enough"), r.Balance),
            DartsBetError.AlreadyPending => string.Format(Loc("bet.already_pending"), r.PendingAmount),
            _ => Loc("bet.failed"),
        };
        try
        {
        ...
        long chatId, ReplyParameters reply)
    {
        ...
        var text = r.Payout > 0
            ? string.Format(Loc("throw.win"), r.Face, r.Multiplier, r.Payout, r.Balance)
