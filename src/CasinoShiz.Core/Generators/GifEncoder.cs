...
public static class GifEncoder
{
    ...
    /// Uses a simple GIF89a encoder since SkiaSharp doesn't natively support animated GIF encoding.
    /// Each frame is 64ms (~15.6 FPS) to match the original.
    /// </summary>
    public static byte[] RenderFramesToGif(byte[][] pngBuffers, int width, int height)
    {
        ...
        writer.Write((byte)0xF7); // GCT flag, 256 colors (8 bits)
        writer.Write((byte)0);    // Background color index
        writer.Write((byte)0);    // Pixel aspect ratio

        // Global Color Table (256 entries)
        // We'll use a simple web-safe palette
        ...
        writer.Write((byte)1);    // Loop indicator
        ...
        foreach (var pngBuffer in pngBuffers)
        {
            ...
            writer.Write((ushort)6);  // Delay: 6 centiseconds = 60ms ≈ 64ms
            ...
            // Quantize pixels to palette and LZW encode
